...
<template>
  <div>
    ...
    <DataState :pending="pending" :error="error" :empty="!data?.rows?.length"
      ...
      <!-- 의안 헤더 -->
      <section class="rounded-2xl bg-white card-shadow p-6 sm:p-7 mb-4">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <div class="relative flex-1">
            ...
            <input
              ...
              class="w-full h-11 rounded-xl border-0 bg-white pl-10 pr-3 text-[14px] card-shadow focus:outline-none focus:ring-2 focus:ring-toss-blue/40"
            />
          </div>
          <select
            v-model="partyFilter"
            class="h-11 rounded-xl border-0 bg-white px-3 text-[14px] font-medium text-toss-gray-700 card-shadow focus:outline-none focus:ring-2 focus:ring-toss-blue/40"
          >
            <option>전체</option>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <div
          ...
          :key="r.name + i"
          class="flex items-center gap-3 rounded-xl bg-white px-4 py-3 card-shadow"
        >
          <div
